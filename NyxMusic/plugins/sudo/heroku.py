...
from NyxMusic.misc import HAPP, SUDOERS, XCB, db
...
from NyxMusic.utils.decorators import AdminActual, language
...
async def vardel_(client, message, _):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            ...
            await message.reply_text(_["heroku_7"].format(check_var))
            os.system(f"kill -9 {os.getpid()} && python3 -m NyxMusic")
...
async def reboot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Nyx.stop_stream(message.chat.id)
    except:
        pass
