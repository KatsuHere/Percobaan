...
from NyxMusic import Apple, Spotify, YouTube, app
from NyxMusic.core.call import Nyx
from NyxMusic.misc import SUDOERS, db
from NyxMusic.utils import seconds_to_min, time_to_seconds
from NyxMusic.utils.channelplay import get_channeplayCB
from NyxMusic.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from NyxMusic.utils.decorators import ActualAdminCB
from NyxMusic.utils.decorators.language import languageCB
from NyxMusic.utils.formatters import seconds_to_min
from NyxMusic.utils.inline.play import (
    livestream_markup,
    panel_markup_1,
    panel_markup_2,
    panel_markup_3,
...
from NyxMusic.utils.stream.autoclear import auto_clean
from NyxMusic.utils.stream.stream import stream
from NyxMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Nyx.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Nyx.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Nyx.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Nyx.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Nyx.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await Nyx.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Nyx.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
...
@app.on_callback_query(filters.regex("NyxPlaylists") & ~BANNED_USERS)
@languageCB
