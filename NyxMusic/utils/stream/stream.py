...
import config
from NyxMusic import Carbon, Saavn, YouTube, app
from NyxMusic.core.call import Nyx
from NyxMusic.misc import db
from NyxMusic.utils.database import (
    add_active_video_chat,
    is_active_chat,
    is_video_allowed,
)
from NyxMusic.utils.exceptions import AssistantErr
from NyxMusic.utils.inline.play import stream_markup, telegram_markup
from NyxMusic.utils.inline.playlist import close_markup
from NyxMusic.utils.pastebin import Nyxbin
from NyxMusic.utils.stream.queue import put_queue, put_queue_index
from NyxMusic.utils.thumbnails import gen_qthumb, gen_thumb
...
async def stream(
    ...
    if forceplay:
        await Nyx.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if streamtype == "saavn_track":
            ...
            if await is_active_chat(chat_id):
                ...
                if not forceplay:
                    db[chat_id] = []
                await Nyx.join_call(chat_id, original_chat_id, file_path, video=None)
                await put_queue(
                    chat_id,
                    original_chat_id,
                    file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Nyx.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
